...
def main(opt):
    ...
    for epoch in range(start_epoch, opt.epoch):
        ...
        epoch += 1
        iters = train_one_epoch(model, loss_func, optimizer, data_loader, sampler, epoch, iters, total_iters, iters_pre_epoch, opt)
        ...
        if epoch % opt.val_every_epoch == 0:
            if (opt.val_dataset != 'None') & (opt.local_rank==0):
                JF_mean = val_one_epoch(model, val_data_loader, epoch, opt, opt.val_dataset)
