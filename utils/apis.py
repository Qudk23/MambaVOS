...
def train_one_epoch(model, loss_func, optimizer, data_loader, sampler, epoch, iters, total_iters, iters_pre_epoch, opt):
    model.train()
    optimizer.zero_grad()
...
def val_one_epoch(model, val_data_loader, epoch, opt, val_dataset):
    model.eval()
    with torch.no_grad():
